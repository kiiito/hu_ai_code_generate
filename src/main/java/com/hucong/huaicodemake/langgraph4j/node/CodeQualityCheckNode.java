...
import com.hucong.huaicodemake.langgraph4j.ai.CodeQualityCheckService;
...
public class CodeQualityCheckNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            // 3. 如果质检失败，增加重试次数并判断是否超过限制
            if (!qualityResult.getIsValid()) {
                int newRetryCount = context.getRetryCount() + 1;
                context.setRetryCount(newRetryCount);
                log.info("代码质检失败，当前重试次数：{}/{}", newRetryCount, context.getMaxRetryCount());

                // 判断是否超过最大重试次数
                if (newRetryCount > context.getMaxRetryCount()) {
                    log.error("超过最大重试次数 {}，终止流程", context.getMaxRetryCount());
                    context.setErrorMessage("代码质量检查失败，已超过最大重试次数");
                } else {
                    log.info("将进行第 {} 次重试", newRetryCount + 1);
                }
            }
