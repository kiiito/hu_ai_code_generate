package com.hucong.huaicodemake.langgraph4j;
...
import com.hucong.huaicodemake.langgraph4j.node.*;
...
public class CodeGenWorkflow {
    ...
    private String routeAfterQualityCheck(MessagesState<String> state) {
        ...
        // 检查是否超过最大重试次数
        if (context.getErrorMessage() != null && context.getErrorMessage().contains("已超过最大重试次数")) {
            log.error("检测到超过最大重试次数，终止流程");
            return "error";
        }

        // 如果质检失败且未超过重试次数，重新生成代码
        if (qualityResult == null || !qualityResult.getIsValid()) {
            log.info("代码质检失败，需要重新生成代码");
            return "fail";
        }
